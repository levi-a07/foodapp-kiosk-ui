...
export class AddMenuComponent implements OnInit {
  ...
  deleteProduct(id: any) {
    this.menu_obj.foodProducts = this.menu_obj.foodProducts.filter(
      (item: any) => item.id != id
    );
    this.result = this.added.filter(
      (x: any) => !this.menu_obj.foodProducts.includes(x)
    );
  }
}
